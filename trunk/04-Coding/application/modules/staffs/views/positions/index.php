	...
	<div class="filter">
		<form class="form-inline" role="form" method="post" action="<?php echo base_url(); ?>staffs/positions/index">
			<div class="form-group">
				<label class="sr-only" for="sta_pos_title">Title in Latin</label>
				<input type="text" class="form-control input-sm" id="sta_pos_title" name="sta_pos_title" value="<?php echo set_value('sta_pos_title'); ?>" placeholder="Title in Latin">
			</div>
			<div class="form-group">
				<label class="sr-only" for="sta_pos_title_kh">Title in Khmer</label>
				<input type="text" class="form-control input-sm" id="sta_pos_title_kh" name="sta_pos_title_kh" value="<?php echo set_value('sta_pos_title_kh'); ?>" placeholder="Title in Khmer">
			</div>
			<div class="form-group">
				<label class="sr-only" for="sta_pos_status">Status</label>
				<?php echo form_dropdown('sta_pos_status', array('' => '-- All Status --', '1' => 'Enabled', '0' => 'Disabled'), set_value('sta_pos_status', $this->session->userdata('sta_pos_status')), 'class="form-control input-sm"') ?>
			</div>
				...
				<tbody>
					<?php if ($data->num_rows() > 0) { ?>
						<?php foreach ($data->result_array() as $row) { ?>
							...
							<tr>
								<td><input type="checkbox" name="id[]" value="<?php $row['sta_pos_id'] ?>" class="checkid" /></td>
								<td><?php echo $row['sta_pos_title']; ?></td>
								<td><?php echo $row['sta_pos_title_kh']; ?></td>
								<td><?php echo status_string($row['sta_pos_status']); ?></td>
								<td><?php echo get_date_string($row['sta_pos_created']); ?></td>
								<td><?php echo get_date_string($row['sta_pos_modified']); ?></td>
								<td>
