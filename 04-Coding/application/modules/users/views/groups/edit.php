...
// loop assigned task
$tasks_assigned = NULL;
if($grouptask->num_rows() > 0){
    foreach ($grouptask->result() as $v){
        $tasks_assigned[$v->grotas_taskid] = $v->grotas_taskid;
    }
}
                                        ...
                                        <?php if(!empty($tass[$con['con_id']])){ ?>
                                            <?php foreach ($tass[$con['con_id']] as $tas): ?>
                                                <tr data-tt-id="<?php echo $tas['mod_id'].'-'.$tas['con_id'].'-'.$tas['tas_id']; ?>" data-tt-parent-id="<?php echo $tas['mod_id'].'-'.$tas['con_id']; ?>">
                                                  <td>
                                                      <label>
                                                          <input type="checkbox" name="tas_id[]" value="<?php echo $tas['tas_id']; ?>" <?php echo set_checkbox('tas_id[]', $tas['tas_id'], (!empty($tasks_assigned[$tas['tas_id']]))?TRUE:FALSE); ?>>
